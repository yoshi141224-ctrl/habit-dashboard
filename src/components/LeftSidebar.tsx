...
const QUOTES = [
  { text: '目標と達成をつなぐ橋は、規律である。', author: 'ジム・ローン' },
  { text: '日々の小さな改善が、長期的な大きな結果につながる。', author: '名言' },
  { text: '先に進む秘訣は、まず始めることだ。', author: 'マーク・トウェイン' },
  { text: '成功とは、日々繰り返される小さな努力の積み重ねだ。', author: 'R. コリアー' },
  { text: '歩みが遅くても、立ち止まらなければいい。', author: '孔子' },
  { text: '行動はすべての成功の基盤である。', author: 'パブロ・ピカソ' },
  { text: 'モチベーションはスタートさせる。習慣は続けさせる。', author: 'ジム・ライアン' },
];

const DAY_JP = ['日', '月', '火', '水', '木', '金', '土'];

const NAV_ITEMS = [
  { id: 'home',      d: 'M3 9l9-7 9 7v11a2 2 0 0 1-2 2H5a2 2 0 0 1-2-2z M9 22V12h6v10', label: 'ホーム' },
  { id: 'calendar',  d: 'M8 2v4 M16 2v4 M3 10h18 M5 4h14a2 2 0 0 1 2 2v14a2 2 0 0 1-2 2H5a2 2 0 0 1-2-2V6a2 2 0 0 1 2-2z', label: 'カレンダー' },
  { id: 'habits',    d: 'M9 11l3 3L22 4 M21 12v7a2 2 0 0 1-2 2H5a2 2 0 0 1-2-2V5a2 2 0 0 1 2-2h11', label: '習慣' },
  { id: 'stats',     d: 'M18 20V10 M12 20V4 M6 20v-6', label: '統計' },
];
...
export default function LeftSidebar({ completionRate, completedCount, totalCount, streak, donutSegments, activeNav, onNavChange, gcalConnected, onGcalConnect }: Props) {
  ...
  const dateStr = `${time.getMonth() + 1}月${time.getDate()}日（${DAY_JP[time.getDay()]}）`;

  const r = 54, cx = 64, cy = 64;
  const circumference = 2 * Math.PI * r;
  const dashoffset = circumference * (1 - completionRate / 100);
  const scoreLabel = completionRate >= 80 ? 'よくできました！' : completionRate >= 50 ? '続けましょう！' : 'まず始めよう！';
      ...
      <aside className={`ls-root${mobileOpen ? ' ls-root--open' : ''}`}>
        ...
        <div className="ls-nav-rail">
          ...
          {gcalConnected ? (
            <button
              type="button"
              className="ls-nav-icon ls-nav-gcal-connected"
              title="Google カレンダー連携済み"
            ...
            <button
              type="button"
              className="ls-nav-icon ls-nav-gcal-disconnected"
              title="📅 カレンダー連携"
              onClick={onGcalConnect}
            >
              <svg width="20" height="20" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round">
          ...
          {/* Settings */}
          <button type="button" className="ls-nav-icon" title="設定">
            <svg width="20" height="20" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round">
        ...
        <div className="ls-content">
          ...
          <div className="ls-score-section">
            <p className="ls-score-title">今日の習慣スコア</p>
            ...
            <p className="ls-score-detail">{completedCount} / {totalCount} 習慣完了</p>
          </div>
          ...
          {streak > 0 && (
            <div className="ls-streak">
              ...
              <span className="ls-streak-label">日連続！</span>
            </div>
          )}

          {/* Daily Time Spent donut */}
          <div className="ls-donut-section">
            <p className="ls-score-title">今日の集中時間</p>
